...
<template>
    <div id="flex-col" class="text-center flex-col">
        ...
        <div v-if="savedCards.length > 0" class="flex-col" style="width: 100%">
            ...
            <div v-else class="flex-col" style="width: 95%">
                <br>
                <div v-for="points of allPoints()" :key="points" style="width: 100%">
                    <grid-view :columns="20" :col-gap="0" :row-gap="0" style="width: 100%">
                        <div v-for="card of cardPointsSort(points)" :key="card.id" style="position: relative">
                            <card-modal :card-id="card.id" :rarity="'Common'" :src="getPicSmallUrl(card.id)"
                                :card-name="card.name" />
                        </div>
                    </grid-view>
                    <span v-for="card of cardPointsSort(points)" :key="card.id" style="position: relative">"{{
                        card.name
                    }}" |
                    </span>
                    <hr>
    ...
    methods: {
        ...
        allPoints() {
            ...
            for (const key in points) {
                if (ris.includes(points[key])) continue
                else ris.push(points[key])
            }
            ris.sort((a, b) => b - a)
            return ris
        },
        cardPointsSort(points) {
            return this.categorySort(this.allcards.filter(_ => _.points === points))
        },
