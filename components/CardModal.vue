...
<template>
	<div style="position: relative">
		...
		<div v-if="getPoints() !== 0" class="svelte-m1ek2q" aria-label="1">{{ getPoints() }}</div>
		<img v-if="limitImage !== ''" :style="{
			position: 'absolute',
			zIndex: 9,
			width: '30%',
			left: '0%',
		...
		<div class="image-loader flex-col">
			<card-rarity class="small-image" :src="src" :rarity="rarity" :card-id="cardId"
				@click.native="toggleFullImage()" />
			<transition name="fade">
				<div v-show="fullImage" class="modal flex-col" oncontextmenu="return false;"
					@mousedown="rightClickClose">
					<button class="modal-close" aria-label="close-modal" @click="toggleFullImage()">
						<x-icon />
						<p>O TASTO DESTRO<br />PER CHIUDERE!</p>
					</button>
					<h1 ref="name" @click="copyName()"></h1>
					<div class="modal-view flex-row">
						<div class="full-image flex-col" style="justify-content: flex-start">
							<a :href="getPicUrl(cardId)" target="_blank">
								<img loading="lazy" :src="getPicUrl(cardId)" :alt="cardId" style="
										display: block;
										margin-left: auto;
										margin-right: auto;
									" />
							</a>
							<grid-view :columns="2" :col-gap="5" :row-gap="0" style="width: 70%">
								<button-secondary :title="'CARD ART'" @click.native="
									$el.querySelector(
										'.full-image img'
								...
								$el.querySelector(
									...
									" />
								<button-secondary :title="'FULL ART'" @click.native="
									$el.querySelector(
										'.full-image img'
						...
						<div v-if="loading" class="loader" style="align-self: flex-start; margin-top: 10vw"></div>
						<div v-if="!loading" class="effects">
							...
							<grid-view :columns="4" :row-gap="0" :col-gap="0">
								<container-pack-scroll v-for="set of cardSets" :key="set.set_code" :set="set"
...
export default {
	...
	methods: {
		...
		getPoints() {
			const points = this.getPointsJSON()
